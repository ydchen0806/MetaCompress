...
def read_data_batch(data_path_list: List[str]) -> np.ndarray:
    ...
    for data_path in data_path_list:
        postfix = os.path.splitext(data_path)[-1]
        if postfix in ['.tif','.tiff']:
            data = imread(data_path)

            if data.ndim ==  4:
                [d, h, w, c] = data.shape
                data = np.reshape(data, [d, h, w])

            data = data[None,None,...]
        elif postfix in ['.mrc','.mrcs']:
            data = mrcfile.open(data_path,permissive=True)
            data = np.array(data.data)
            data = data[None,None,...]
        elif postfix in ['.nii','.nii.gz']:
...
def save_data_batch(data_batch: np.ndarray,save_path_list: List[str]):
    ...
    for idx,save_path in enumerate(save_path_list):
        ...
        if postfix in ['.tif','.tiff']:
            ...
            data = data_batch[idx,0,...]
            with mrcfile.new(save_path) as mrc:
                mrc.set_data(data)
        else:
            raise NotImplementedError
...
if __name__ == '__main__':
    data_path = '/braindat/lab/chenyd/DATASET/COVID19_CT/CT-2'
    data_list = gen_data_path_list_list(data_path,10, drop_last=True)
    data_batch = read_data_batch(data_list[0])
    print('done!')