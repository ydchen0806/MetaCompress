...
class SIREN(nn.Module):
    ...
    """
    def __init__(self, coords_channel=3,data_channel=1,features=256,layers=5,w0=30,res=False,output_act=False):
        super().__init__()
        self.net=[]
...
class MFNGabor(MFNBase):
    ...
    def calc_features(param_count,coords_channel,data_channel,layers,**kwargs):
        ...
        features = round((-b+math.sqrt(b**2-4*a*c))/(2*a))
        return features
ALLPHI={'SIREN':SIREN,'FFN':FFN,'NeRF':NeRF,'MFNFourier':MFNFourier,'MFNGabor':MFNGabor}
ALL_CALC_PHI_FEATURES = {'SIREN':SIREN.calc_features,'FFN':FFN.calc_features,'NeRF':NeRF.calc_features,'MFNFourier':MFNFourier.calc_features,'MFNGabor':MFNGabor.calc_features}
ALL_CALC_PHI_PARAM_COUNT = {'SIREN':SIREN.calc_param_count,'FFN':FFN.calc_param_count,'NeRF':NeRF.calc_param_count,'MFNFourier':MFNFourier.calc_param_count,'MFNGabor':MFNGabor.calc_param_count}
