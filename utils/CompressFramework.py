    ...
    for idx,data_path_list in enumerate(tqdm(data_path_list_list,'Evaluating performance',leave=False)):
        ...
        if len(orig_data.shape) == 5:
            for i in range(orig_data.shape[0]):
                ...
                d, h, w, _ = orig_data_.shape
                orig_data_ = np.reshape(orig_data_, [d, h, w])
                decompressed_data_ = np.reshape(decompressed_data_, [d, h, w])
                ssim_list.append(structural_similarity(orig_data_,decompressed_data_,data_range=max,channel_axis=None)) 
        else:
            raise NotImplementedError
