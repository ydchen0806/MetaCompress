...
class Test_Random_Init(_BaseCompressFramerwork):
    ...
    def sample_nf(self, coords: torch.Tensor) -> torch.Tensor:
        ...
        return data_hat
    def test(self,save_dir:str,Log:MyLogger):
        ...
        val_steps = self.opt.Compress.val_steps
        checkpoints = list(range(val_steps, max_steps, val_steps))
        checkpoints.append(max_steps)
        def start():
            # evaluate performance
            for checkpoint in checkpoints:
                self.opt.Compress.max_steps = checkpoint
                torch.cuda.empty_cache()
                self.module['phi'].half()
                self.save_module(opj(save_dir,'trained_module','steps_{}.pt'.format(checkpoint)))
                save_model_(self.module['phi'], opj(save_dir,'trained_module_','steps_{}'.format(checkpoint)))
                eval_save_dir = opj(save_dir,'eval_results','steps_{}'.format(checkpoint))
            ...
            'performance/compress_ratio':performance['compress_ratio'].mean(),'epoch':0},checkpoint)   
...
def main():
    ...
    Log.log_opt(opt)
    CompressFramework = Test_Random_Init(opt.CompressFramework)
    CompressFramework.test(Log.logdir,Log)
