...
class FlattenedSampler:
    def __init__(self,data: torch.Tensor, std: np.ndarray, sample_size:int,sample_count:int, use_uncertainty: bool) -> None:
        ...
        if self.use_uncertainty:
            self.weight = self.calc_uncertainty()
    ...
    def calc_uncertainty(self):
        uncertainty_weights = self.std
        uncertainty_weights = uncertainty_weights.flatten()
        uncertainty_weights = 1.0 / (uncertainty_weights + 1e-6)
        uncertainty_weights = uncertainty_weights / np.sum(uncertainty_weights)
        return uncertainty_weights

    def generate_uncertainty_weights(self):
        if self.use_uncertainty:
            sampler_idxs = np.random.choice(self.pop_size, self.sample_size, p=self.weight)
        else:
            sampler_idxs = np.random.choice(self.pop_size, self.sample_size)
        return sampler_idxs
    
    def __next__(self,) -> Tuple[torch.Tensor,torch.Tensor]:
        if self.index < self.__len__():
            sampled_idxs = self.generate_uncertainty_weights()
...
class FlattenedDataset:
    def __init__(self,batch_size:int,Normalize_opt:NormalizeOpt,shuffle_path:bool=True,data_dir:str=None,data_path_list:List[str]=None) -> None:
        ...
        self.total_data = read_data_batch(self.data_path_list)
        if isinstance(self.total_data, np.ndarray):
            self.std_volume = np.std(self.total_data, axis=(1, 2, 3, 4))
            self.std_pixel = np.std(self.total_data, axis=0)
        elif isinstance(self.total_data, torch.Tensor):
            self.std_volume = torch.std(self.total_data, dim=(1, 2, 3, 4))
            self.std_pixel = torch.std(self.total_data, dim=0)
        else:
            raise NotImplementedError
        del self.total_data
    def __len__(self):
...
def MAML_train(args):
    ...
    train_std = dataset.std_pixel
    val_std = val_dataset.std_pixel
    # define the model
    ...
    meta_phi = accelerate.prepare(meta_phi)
    ...
    step = 0
    for i in tqdm(range(opt.Train.maml_epoch)):
        for (data,sideinfos) in dataset:
            step += 1
            data = data.to(device)
            flattened_sampler = FlattenedSampler(data, train_std, opt.Train.sample_size, opt.Train.sample_count, opt.Train.use_uncertainty_sampler)
            flattened_sampler = accelerate.prepare(flattened_sampler)
                ...
                if not step % opt.Train.val_every_n_step:
                    ...
                    with torch.no_grad():
                        for val_step, (val_data, sideinfos) in enumerate(val_dataset):
                            ...
                            val_flattened_sampler = FlattenedSampler(val_data, val_std, opt.Train.sample_size, opt.Train.sample_count, opt.Train.use_uncertainty_sampler)
